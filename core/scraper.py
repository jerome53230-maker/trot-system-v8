...
class PMUScraper:
    ...
    def __init__(self):
        ...
        logger.info("⚠️ Scraper initialisé SANS cache (mode debug)")
    
    def get_race_data(self, date_str: str, reunion: int, course: int) -> Optional[Race]:
        """
        Récupère les données complètes d'une course (SANS cache - debug mode).
        ...
        logger.info(f"⚠️ MODE DEBUG - Pas de cache, fetch direct API")
